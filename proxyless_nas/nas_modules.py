...
class MobilenetV1(BasicUnit):
	...
	def build_from_config(config):
		...
		return MobilenetV1(first_conv, blocks, feature_mix_layer, classifier)
	...
	def keys_parameters(self, keys):
		for name, param in self.named_parameters():
			for key in keys:
				if key in name:
					yield param

	def not_keys_parameters(self, keys):
		for name, param in self.named_parameters():
			flag = True
			for key in keys:
				if key in name:
	...
	def _make_divisible(v, divisor, min_val=None):
		...
		It can be seen here:
		https://github.com/tensorflow/proxyless_nas/blob/master/research/slim/nets/mobilenet/mobilenet.py
		:param v:
		:param divisor:
		:param min_val:
		:return:
	...
	def adjust_width(self, width_type, width_mult):
		...
		last_channel = self._make_divisible(1280 * width_mult, 8) if width_mult > 1.0 else 1280
		...
		net = self.build_from_config(self.config)
		net.set_bn_param(bn_momentum=net_config['bn']['momentum'], bn_eps=net_config['bn']['eps'])
		return net


class MobileNetV2(MobilenetV1):
...
class MnasNet(MobilenetV1):

	def __init__(self, n_classes=1000, width_mult=1, bn_param=(0.1, 1e-5), dropout_rate=0.2, **kwargs):
		input_channel = 32
		last_channel = 1280
		...
		for t, c, n, s, k in interverted_residual_setting:
			...
			for i in range(n):
				if i == 0:
					...
					stride = 1
				mobile_inverted_conv = MBInvertedConvLayer(
					in_channels=input_channel, out_channels=output_channel, kernel_size=k, stride=stride,
					expand_ratio=t, has_final_relu=False,
				)
		...
		super(MnasNet, self).__init__(first_conv, blocks, feature_mix_layer, classifier)
		# set bn param
		self.set_bn_param(bn_momentum=bn_param[0], bn_eps=bn_param[1])
